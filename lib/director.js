...
const read = (text) => {
  ...
  for (let t of tokens) {
    ...
    if (t.type === 'image') {
      const optionParseResult = /.*<!--\s*({.*})\s*-->/.exec(t.text)
      const options = (optionParseResult) ? JSON.parse(optionParseResult[1]) : null
      img = assign({}, t, options)
      if (img.wait) {
        i += 1
        actions.push({
          id: i,
    ...
    actions.push({
      ...
      img: img,
      text: t.text,
      options: assign({}, currentOptions, singleActionOptions)
    })
  }
  return {
    title: title,
