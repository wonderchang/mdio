...
const Mdio = class {
  constructor (options) {
    ...
    this.player.progress.onclick = (evt) => {
      const ratio = evt.clientX / this.player.progress.clientWidth
      const actionI = Math.round(this.show.length * ratio)
      this.actionI = (actionI < 1) ? 0 : actionI - 1
      this._transition()
    }
